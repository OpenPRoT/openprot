...
use openprot_attest_api::{
    AttestConfig, AttestError, AttestProducer, HwSigner, MeasurementProvider, SwSigner,
};
...
// ── Software-key producer ─────────────────────────────────────────────────────

/// Attestation producer backed by a caller-supplied P-384 private scalar and
/// certificate chain.
///
/// Unlike [`HwAttestProducer`], this path does not enforce DICE chain
/// requirements (length, MultiTcbInfo extension) because the certificates are
/// not generated by the Caliptra DICE engine.  The UEID field in the token is
/// derived from the leaf certificate if it carries the TCG UEID extension;
/// otherwise a zero-filled placeholder is used.
pub struct SwAttestProducer<'a> {
    signer: SwSigner,
    config: AttestConfig,
    providers: Vec<&'a dyn MeasurementProvider, MAX_PROVIDERS>,
}

impl<'a> SwAttestProducer<'a> {
    pub fn new(signer: SwSigner, config: AttestConfig) -> Self {
        Self {
            signer,
            config,
            providers: Vec::new(),
...
impl AttestProducer for SwAttestProducer<'_> {
    fn generate_token(
        &self,
        nonce: &[u8],
        out: &mut Vec<u8, MAX_TOKEN_SIZE>,
