...
fn flash_server() -> Result<(), ErrorCode> {
    pw_log::info!("flash_server: initializing driver");
    let mut driver =
    ...
    pw_log::info!("flash_server: entering loop");
    loop {
        syscall::object_wait(
...
fn entry() -> Result<(), Error> {
    pw_log::info!("🔄 FLASH SERVER START");
    let ret = flash_server();

    // Log that an error occurred so that the app that caused the shutdown is logged.
    let ret = match ret {
        Ok(()) => {
            pw_log::info!("✅ FLASH SERVER PASS");
            Ok(())
        }
        Err(e) => {
            pw_log::error!("❌ FLASH SERVER FAIL: {:08x}", u32::from(e) as u32);
            Err(Error::Unknown)
        }
    };
