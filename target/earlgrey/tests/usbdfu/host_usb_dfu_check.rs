...
fn run_dfu_test(transport: &TransportWrapper, usb: &UsbOpts) -> Result<()> {
    ...
    log::info!(
        "waiting for DFU device (VID={:04x}, PID={:04x}, Serial={})...",
        usb_vid,
        usb_pid,
        serial_number
    );
    ...
    for alt in 1..=3 {
        ...
        match dfu.upload(0, &mut cert_buf) {
            Ok(n) => {
                ...
                if n > 0 {
                    log::info!(
                        "Cert Alt {} data (truncated): {:?}",
                        alt,
                        &cert_buf[..std::cmp::min(n, 16)]
                    );
                } else {
...
fn main() -> Result<()> {
    ...
    run_dfu_test(&transport, &args.usb)?;
    Ok(())
}
