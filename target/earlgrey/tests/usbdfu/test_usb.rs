            ...
            Ok((cert, _)) => {
                if n == 0 {
                    ...
                    if len <= data.len() {
                        ...
                        pw_log::error!(
                            "Certificate is bigger than buffer ({} bytes)",
                            data.len() as usize
                        );
                        break;
                    }
    ...
    fn upload(&mut self, alt: u8, block_num: u16, data: &mut [u8]) -> Result<usize, DfuStatus> {
        ...
        match alt {
            0 => {
                ...
                let len = core::cmp::min(self.written_bytes.saturating_sub(offset), block_size);
                if len == 0 {
                    return Ok(0);
                }
                let chunk = data.get_mut(..len).ok_or(DfuStatus::ErrUnknown)?;
                self.flash
                    .read(FlashAddress::data(TEST_FLASH_START + offset as u32), chunk)
                    .map_err(|_| DfuStatus::ErrUnknown)?;
                Ok(len)
            }
