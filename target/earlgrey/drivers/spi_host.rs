...
impl SpiHost {
    /// Create a new SpiHost instance using the peripheral ownership token for SpiHost0.
    pub fn new_spi_host0(token: spi_host::SpiHost0) -> Self {
        let _ = token;
        Self {
            mmio: unsafe { spi_host::RegisterBlock::new(spi_host::SpiHost0::PTR) },
        }
    }

    /// Create a new SpiHost instance using the peripheral ownership token for SpiHost1.
    pub fn new_spi_host1(token: spi_host::SpiHost1) -> Self {
        let _ = token;
        Self {
            mmio: unsafe { spi_host::RegisterBlock::new(spi_host::SpiHost1::PTR) },
        }
    }
