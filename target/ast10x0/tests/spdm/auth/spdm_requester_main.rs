...
fn run() -> Result<(), u32> {
    ...
    loop {
        ...
        generate_capabilities_request_local(requester.context_mut(), &mut buf).map_err(|_| {
            pw_log::error!("generate_capabilities_request failed on round {}", round as u32);
            7u32
        })?;
        requester
            .context_mut()
        ...
        buf.reset();
        generate_get_certificate(requester.context_mut(), &mut buf, 0, 0, MAX_CERT_SIZE, false)
            .map_err(|_| {
        ...
        requester.context_mut().get_random_bytes(&mut nonce).map_err(|_| {
        ...
        .map_err(|_| {
            pw_log::error!("generate_challenge_request failed on round {}", round as u32);
            20u32
        })?;
        requester
            .context_mut()
