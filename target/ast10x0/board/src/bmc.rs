...
const CHECKPOINTS: &[BootCheckpoint<BmcSignal>] = &[BootCheckpoint::new(
    "bmc-ready",
    BmcSignal::Ready,
    READY_WINDOW,
)];
...
pub const BMC: DeviceConfig<BmcReset, BmcSignal> =
    DeviceConfig::new("bmc", BmcReset::Bmc, CHECKPOINTS);
...
/// Resolves this board's signal ids to evidence.
///
...
impl<P: InputPin> EvidenceReader<BmcSignal> for BmcReader<P>
where
    P::Error: 'static,
{
    ...
    fn read(&mut self, signal: &BmcSignal) -> Result<BootStatus, Self::Error> {
        match signal {
            BmcSignal::Ready => self.ready.boot_status(),
        }
    }
}
...
mod tests {
    ...
    fn the_table_declares_one_ready_checkpoint() {
        ...
        assert_eq!(*checkpoints[0].signal(), BmcSignal::Ready);
    ...
    fn the_reader_resolves_ready_to_the_line_level() {
        ...
        assert_eq!(
            booting.read(&BmcSignal::Ready).expect("read failed"),
            BootStatus::Booting
        );
        assert_eq!(
            booted.read(&BmcSignal::Ready).expect("read failed"),
            BootStatus::Booted
        );
    }
    ...
    fn the_reader_answers_every_signal_the_table_names() {
        ...
        for checkpoint in BMC.checkpoints() {
            assert_eq!(
                reader.read(checkpoint.signal()).expect("read failed"),
                BootStatus::Booted
            );
        }
    }
    ...
    fn walk<P: InputPin>(ready: P) -> CheckpointWalk<BmcReader<P>, BmcSignal>
    where
        P::Error: 'static,
    {
        CheckpointWalk::new(BmcReader::new(ready), BMC.checkpoints())
    }
