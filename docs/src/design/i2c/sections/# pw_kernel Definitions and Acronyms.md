# pw_kernel Definitions and Acronyms

This document provides definitions for key concepts, objects, and acronyms used
in the Pigweed kernel (pw_kernel).

## Acronyms

| Acronym | Expansion | Description |
|---------|-----------|-------------|
| **ABI** | Application Binary Interface | The low-level interface between user space and kernel |
| **IRQ** | Interrupt Request | A hardware signal requesting processor attention |
| **IPC** | Inter-Process Communication | Mechanism for processes/apps to communicate |
| **MPU** | Memory Protection Unit | Hardware for enforcing memory access permissions (ARM) |
| **MAIR** | Memory Attribute Indirection Register | ARM register for memory type configuration |
| **NVIC** | Nested Vectored Interrupt Controller | ARM Cortex-M interrupt controller |
| **PLIC** | Platform-Level Interrupt Controller | RISC-V interrupt controller |
| **PMP** | Physical Memory Protection | Hardware for memory protection (RISC-V) |
| **PMSAv7** | Protected Memory System Architecture v7 | ARM MPU architecture for ARMv7-M |
| **PMSAv8** | Protected Memory System Architecture v8 | ARM MPU architecture for ARMv8-M |
| **SRD** | Sub-Region Disable | PMSAv7 feature for fine-grained memory control |
| **SVC** | Supervisor Call | ARM instruction for syscalls |
| **TEX** | Type Extension | PMSAv7 memory attribute field |
