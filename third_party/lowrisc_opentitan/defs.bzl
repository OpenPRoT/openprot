...
def opentitan_rust_binary(name, actual = None, **kwargs):
    if actual:
        ...
        if "manual" not in raw_tags:
            # We must copy the list to avoid modifying the original if it was shared (unlikely but safe)
            raw_tags = list(raw_tags) + ["manual"]
        rust_binary(
            name = raw_name,
            **dict(kwargs, tags = raw_tags)
        )
        _opentitan_rust_binary_rule(
            name = name,
            actual = ":" + raw_name,
